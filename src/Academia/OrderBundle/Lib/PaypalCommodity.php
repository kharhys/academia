...
namespace Academia\OrderBundle\Lib;
 ...
 *
 */
abstract class PaypalCommodity {
     ...
     *
     * @param array $args
     * @internal param $api_credentials , required, a name => value array containing your API username, password and signature.
     * @internal param $args , named parameters to customise the subscription and checkout process. See description for available parameters.
     */
    public function __construct( $args = array() ){

        if( '' == PaypalConfiguration::username() || '' == PaypalConfiguration::password() || '' == PaypalConfiguration::signature() )
            exit( 'You must specify your PayPal API username, password & signature in the $api_credentials array. For details of how to ' );
        elseif( ( empty( $args['return_url'] ) && '' == PaypalConfiguration::return_url() ) || ( empty( $args['cancel_url'] ) && '' == PaypalConfiguration::cancel_url() ) )
        ...
        $defaults = array(
            ...
            'business_name' => '',
            'solution_type' => 'Sole',
            'return_url'    => PaypalConfiguration::return_url(),
            'cancel_url'    => PaypalConfiguration::cancel_url(),
            'notify_url'    => PaypalConfiguration::notify_url()
        );

        $args = array_merge( $defaults, $args );

        $this->currency      = PaypalConfiguration::currency();
    ...
    protected function get_api_credentials_url(){
        ...
        return 'USER=' . urlencode( PaypalConfiguration::username() )
        . '&PWD=' . urlencode( PaypalConfiguration::password() )
        . '&SIGNATURE=' . urlencode( PaypalConfiguration::signature() )
        . '&VERSION='.  urlencode( PaypalConfiguration::version() );
    }
     ...
     * @param string $profile_or_transaction_id
     * @internal param $profile_id , (optional) string. A PayPal Recurrent Payment Profile ID, required for GetRecurringPaymentsProfileDetails operation.
    ...
    protected function call_paypal( $action, $profile_id = '', $status = '', $args = array() ){
        ...
        $ch = curl_init();
        curl_setopt( $ch, CURLOPT_URL, PaypalConfiguration::endpoint() );
        ...
        if( ( 0 == sizeof( $parsed_response ) ) || ! array_key_exists( 'ACK', $parsed_response ) )
            exit( "Invalid HTTP Response for POST request($api_parameters) to " . PaypalConfiguration::endpoint() );
     ...
     * @param array $args
     * @return string
     * @internal param array $args . Name => value parameters to customise the buy button.
    ...
    public function get_buy_button( $args = array() ){
        ...
        $defaults = array(  'id'        => 'paypal-submit',
            'type'      => 'anchor',
            'href'      => PaypalConfiguration::checkout_url(),
            'alt'       => 'Submit',
            'get_token' => true,
            'locale'    => 'en_US'
        );
        ...
        if( $args['type'] == 'anchor' ) {
            ...
            if( $args['href'] == PaypalConfiguration::checkout_url() )
    ...
    public function get_checkout_url() {
        ...
        // Include the token in the href if the default href is not overridden
        return PaypalConfiguration::checkout_url() . $this->token;
    }
    ...
    public function get_currency_symbol( $currency_code = '', $echo = false ){
        ...
        if( empty( $currency_code ) )
            $currency_code = PaypalConfiguration::currency();
